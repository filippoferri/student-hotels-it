import React from 'react';
import axios from 'axios';

const URL_BASE = 'https://api-crt.cert.havail.sabre.com';
const SIMPLE_HOTEL_SEARCH_ENDPOINT = `/v1.0.0/shop/hotels/content?mode=content`;

const instance = axios.create({
  baseURL: URL_BASE,
  headers: {
    'Authorization': 'Bearer ',
    'X-Originating-IP': '195.96.201.90',
    'Content-Type': 'application/json'
  }
});

export default {
  HotelContentAPI: params =>
    instance.post(SIMPLE_HOTEL_SEARCH_ENDPOINT, { params }),
};


